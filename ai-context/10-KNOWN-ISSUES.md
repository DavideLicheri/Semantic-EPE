...
ssh <VM_USER>@<VM_IP> "systemctl status eces-backend"

# Check logs
ssh <VM_USER>@<VM_IP> "sudo journalctl -u eces-backend -n 50"

# Test API
curl http://<VM_IP>/api/auth/login -X POST \
...
```bash
# Test API
curl http://<VM_IP>/api/euring/versions/matrix

# Check JSON files
ssh <VM_USER>@<VM_IP> "ls -la /opt/eces/backend/data/euring_versions/versions/"
```
...
curl http://<VM_IP>/api/euring/recognize -X POST \
  -H "Content-Type: application/json" \
  -d '{"euring_string":"YOUR_STRING"}'

# Test conversione
curl http://<VM_IP>/api/euring/convert -X POST \
  -H "Content-Type: application/json" \
...
```bash
# Check PostgreSQL
ssh <VM_USER>@<VM_IP> "systemctl status postgresql"

# Check database
ssh <VM_USER>@<VM_IP> "psql -U eces_user -d eces -c 'SELECT COUNT(*) FROM query_log;'"
