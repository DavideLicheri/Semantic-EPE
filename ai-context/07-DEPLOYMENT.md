...
sudo chown -R <VM_USER>:<VM_USER> /opt/eces
...
# Check Nginx status
ssh <VM_USER>@<VM_IP> "systemctl status nginx"
...
ssh <VM_USER>@<VM_IP> "sudo tar -czf /opt/eces/frontend-backup-$(date +%Y%m%d-%H%M%S).tar.gz -C /opt/eces/frontend ."
...
```bash
# Backup prima di deploy
ssh <VM_USER>@<VM_IP> "tar -czf /opt/eces/backend-backup-$(date +%Y%m%d-%H%M%S).tar.gz -C /opt/eces/backend app/ data/ main.py"
